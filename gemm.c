        ...
        for(int bx = 0; bx < N; bx+=BLOCK_SIZE_X) {
            for(int y = by; y < by+BLOCK_SIZE_Y; y++) {
                for(int x = bx; x < bx+BLOCK_SIZE_X; x+=4) {
                    float32x4_t sum_vec0 = vdupq_n_f32(0.0f); // Initialize to zero
                    float32x4_t sum_vec1 = vdupq_n_f32(0.0f); // Initialize to zero
                    float32x4_t sum_vec2 = vdupq_n_f32(0.0f); // Initialize to zero
                    float32x4_t sum_vec3 = vdupq_n_f32(0.0f); // Initialize to zero
                    for(int k = 0; k < N;  k+=4) {
